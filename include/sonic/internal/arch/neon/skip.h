...
sonic_force_inline bool SkipContainer(const uint8_t *data, size_t &pos,
  ...
  while (pos + VEC_LEN <= len) {
    ...
    uint64_t rbrace = to_bitmask(vceqq_u8(v, vdupq_n_u8(right))) & to_one_mask & not_in_str_mask;
    uint64_t lbrace = to_bitmask(vceqq_u8(v, vdupq_n_u8(left))) & to_one_mask & not_in_str_mask;
    ...
    pos += quote_idx;
    if (quote_bits) { SkipString(data, pos, len); }
  }
